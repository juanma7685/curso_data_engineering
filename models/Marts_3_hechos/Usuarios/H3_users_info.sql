WITH user_data AS (
    ...
    FROM 
        {{ ref('H3_dim_users') }} u
    LEFT JOIN 
        {{ ref('H3_facts_orders') }} o ON u.user_id = o.user_id
    LEFT JOIN {{ref('H3_facts_order_items')}} foi ON foi.order_id = o.order_id
    LEFT JOIN 
        {{ ref('H3_dim_orders') }} d ON o.order_id = d.order_id -- Agregado para usar columnas de `dim_orders`
    GROUP BY 
